...
class SoundcloudScraper(scrapy.Spider):
    ...
    artist_name = 'idealismus'
    artist_home = f'https://soundcloud.com/{artist_name}'

    offset = base_url.split('offset=')[1].split('&')[0]
    offsets = []
    related_artist_list = []
            ...
            for user in collection:
                ...
                if int(user_followers) >= self.MIN_FOLLOWERS and int(user_track_count) >= self.MIN_TRACK_COUNT:
                    ...
                    if follower_ratio <= self.MIN_FOLLOWERS_TO_FOLLOWING:
                        ...
                        artist_social_link = f'https://api-v2.soundcloud.com/users/soundcloud:users:{id_number}/web-profiles?client_id={self.client_id}&app_version={time.time()}&app_locale=en'
                        ...
                        if potential_emails or social_links:
                            ...
                            tracklist_url = f'https://api-v2.soundcloud.com/users/{id_number}/toptracks?client_id={self.client_id}&limit=10&offset=0&linked_partitioning=1&app_version={time.time()}&app_locale=en'
